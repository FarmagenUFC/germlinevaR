#' Cohort Plot from a germline MAF (read.gvr / gvr_filter output)
#'
#' @description
#' Draws a maftools-style plot from a MAF-style table - the output of
...
#'
#' @details
#' This is the standalone plot previously produced inside [gvr_summary()]. It
...
#' If it is not installed the plot is skipped with a warning and `NA_character_`
#' is returned.
#'
...
#'
#' ## write a top-20 plot to the current directory
#' p <- gvr_plot(maf)
#' p                                  # path to the PNG
#'
#' ## smaller plot of filtered hits, into a results folder
...
gvr_plot <- function(maf,
  ...
  # --- Plot builder (ComplexHeatmap): top-N genes x samples, each cell the ---
