...
#' gvr_oncoplot(gvr_filter(maf), top_n = 15, out_dir = "results/plots")
#' }
#'
...
#' @export
gvr_oncoplot <- function(maf,
                         top_n       = 20,
                         sample_col  = "Tumor_Sample_Barcode",
                         out_dir     = ".",
                         file_prefix = "gvr_oncoplot",
                         verbose     = TRUE) {

  if (!requireNamespace("data.table", quietly = TRUE)) {
    stop("gvr_oncoplot requires the 'data.table' package.")
  }
  ...
  if (!sample_col %in% names(dt)) {
    warning(sprintf("gvr_oncoplot: sample column '%s' not found; pooling all rows into 'All'.",
                    sample_col))
    dt[, .__sample__ := "All"]
  } else {
  ...
  if (length(miss_req) > 0) {
    stop(sprintf("gvr_oncoplot: required column(s) not found: %s",
  ...
  .fuse_save_png <- function(final_path, draw_fun) {
    ...
    ok <- tryCatch({ draw_fun(tmp); file.exists(tmp) && file.info(tmp)$size > 0 },
                   error = function(e) {
                     warning(sprintf("gvr_oncoplot: plot render failed (%s): %s",
    ...
    if (!file.exists(final_path) || file.info(final_path)$size == 0) {
      warning(sprintf("gvr_oncoplot: copy to '%s' failed; left at '%s'.", final_path, tmp))
      return(tmp)
    }
    final_path
  }
  ...
  if (!requireNamespace("ComplexHeatmap", quietly = TRUE)) {
    warning("gvr_oncoplot: 'ComplexHeatmap' not installed; skipping oncoplot."); return(invisible(NA_character_))
  }
  m <- dt[!(Hugo_Symbol %in% UNKNOWN_GENE)]
  if (nrow(m) == 0L) { warning("gvr_oncoplot: no known-gene variants; skipping oncoplot."); return(invisible(NA_character_)) }
  ...
  if (!is.na(path) && isTRUE(verbose)) message(sprintf("gvr_oncoplot: written %s", path))
  invisible(path)
}
